...
func (c Public) SignUp() revel.Result {
	var err error
	var count = 0
...
func (c Public) Activate(activation_key string) revel.Result {
	var user models.RegistrationProfile
	var err error
	...
	if err != nil {
		...
		c.Validation.Keep()
		c.FlashParams()
		return c.Redirect(Public.RenderActivation, activation_key)
	}
...
func (c Public) RenderActivation(activation_key string) revel.Result {
	return c.RenderTemplate("App/Activation.html")
}

func (c Public) Login() revel.Result {
	var err error
	...
	err = app.DB.Model(&user).Association("Tokens").Append(token).Error
	if err != nil {
		panic(err)
	}
